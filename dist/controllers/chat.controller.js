...
export const sendMessage = async (req, res) => {
    try {
        ...
        const senderLastSeen = conversation.lastSeen.find((entry) => entry.user.equals(senderId));
        if (senderLastSeen) {
        ...
        const receiverId = conversation.participants.find((id) => !id.equals(senderId));
...
export const getUserConversation = async (req, res) => {
    try {
        ...
        const conversationsWithUnread = await Promise.all(conversations.map(async (conversation) => {
            const lastSeen = conversation.lastSeen.find((entry) => entry.user.equals(userId));
            let unreadCount = 0;
