...
import cloudinary, { removeFromCloudinary, uploadToCloudinary, uploadVideoToCloudinary, } from "../utils/cloudinary.js";
...
export const createPost = async (req, res) => {
    try {
        ...
        if (files["image"] && files["video"]) {
            throw new ApiError(400, "You can upload either an image or a video, not both");
        }
        let image;
        let videoHlsUrl;
        let videoThumbnail;
        ...
        const populatedPost = await Post.findById(createdPost._id).populate("owner", "username profileImage");
        if (!populatedPost) {
        ...
        return res.json(new ApiResponse(200, formattedPost, "post created successfully"));
    }
    catch (error) {
...
export const getAllPostsForHome = async (req, res) => {
    try {
        const userId = req.user?._id;
        ...
        if (cachedData) {
            return res
                .status(200)
                .json(new ApiResponse(200, JSON.parse(cachedData), "post fetched from cache"));
        }
        if (!userId) {
...
export const getUserPosts = async (req, res) => {
    try {
        ...
        if (cachedData) {
            return res
                .status(200)
                .json(new ApiResponse(200, JSON.parse(cachedData), "user post fetched successfully from cache"));
        }
        const posts = await Post.aggregate([
            {
                $lookup: {
