...
import { removeFromCloudinary, uploadToCloudinary, } from "../utils/cloudinary.js";
...
export const registerUser = async (req, res) => {
    try {
        ...
        if (existingUser) {
            throw new ApiError(409, "user with this email or username already exisnt, please try something else");
        }
        let user;
        if (profileImageUrl) {
        ...
        return res
            ...
            .json(new ApiResponse(201, { success: true, user: loggedInUser, accessToken, refreshToken }, "User registered Successfully..."));
    }
    catch (error) {
...
export const refreshAccessToken = async (req, res) => {
    try {
        ...
        return res
            ...
            .json(new ApiResponse(201, { refreshToken: newRefreshToken, accessToken: newAccessToken }, "refresh token successfully created"));
    }
    catch (error) {
        console.log("Error", error);
        const statusCode = error instanceof ApiError ? error.statusCode : 500;
...
export const getUserProfileData = async (req, res) => {
    try {
        ...
        return res
            .status(200)
            .json(new ApiResponse(200, profileData[0], "user data fetched succesfully"));
    }
    catch (error) {
        console.log("Error", error);
        const statusCode = error instanceof ApiError ? error.statusCode : 500;
...
export const followUser = async (req, res) => {
    try {
        ...
        return res
            .status(200)
            .json(new ApiResponse(200, null, `You are following ${userToBeFollowed.username} now`));
    }
    catch (error) {
        console.log("Error", error);
        const statusCode = error instanceof ApiError ? error.statusCode : 500;
...
export const unFollowUser = async (req, res) => {
    try {
        ...
        res
            .status(200)
            .json(new ApiResponse(200, null, `You Unfollowed ${userToUnfollow.username}`));
    }
    catch (error) {
        console.log("Error", error);
        const statusCode = error instanceof ApiError ? error.statusCode : 500;
